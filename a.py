...
def main():
    ...
    last_order_time = transform_time(orders[-1:][0].get("created"))
    last_order_time2 = transform_time(orders[-2:][0].get("created"))

    algorithm_start = datetime.now()

    # 向前计算
    time = algorithm(luckly_number, total_tickets, time_sum, last_order_time2, last_order_time)
    rebot_times.append(time)

    # 向后计算
    time = algorithm(luckly_number, total_tickets, time_sum, last_order_time, None)
    rebot_times.append(time)

    print("机器人应该购买的时间：", rebot_times)

    print("计算耗时", datetime.now() - algorithm_start)


    # time = algorithm(luckly_number, total_tickets, time_sum, last_order_time - 999, last_order_time + 999)
